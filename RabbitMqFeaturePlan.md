...
- [x] G1: Полный функциональный паритет с Service Bus / Storage Queue (отправка одиночных, батчей, с задержкой через UI; сообщения появляются в очереди RabbitMQ).
- [x] G2: Получение сообщений в реальном времени с форматированием JSON и декомпрессией gzip/base64.
- [x] G3: Поддержка сущностей Queue и Exchange (direct/fanout/topic) с routing key и привязкой очереди для получения.
- [x] G4: Ноль изменений в `src/Application`.
- [x] G5: Документация в `readme.md` (docker run, примеры AMQP URI).
- [x] G6: Регрессия существующих шин (Event Hubs / Storage Queue / Service Bus) — приложение собирается и работает.
